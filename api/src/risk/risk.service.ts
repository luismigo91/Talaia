...
export class RiskService {
  ...
  async risk(opts: { station?: string; now?: Date } = {}): Promise<StationRisk[]> {
    ...
    return Promise.all(targets.map((s) => this.riskFor(s, now)));
  }

  private async riskFor(station: VirtualStation, now: Date): Promise<StationRisk> {
    const [points, thresholds] = await Promise.all([
      loadWatchPoints(this.db, station.id),
      loadThresholds(this.db, station.id),
    ]);
    ...
    const flowComponents = await this.flowComponents(hydro, now, warnings);
    components.push(...flowComponents.components);
    components.push(...(await this.rainObservedComponents(rain, thresholds, now)));
    components.push(...(await this.rainForecastComponents(station, thresholds, now)));

    const alerts = await this.alertsFor(station, now);
    ...
    return {
      ...
      alerts,
      warnings,
      stale: flowComponents.anyFresh === false && hydro.length > 0,
      computed_at: now.toISOString(),
    };
  }

  /** Caudal y embalses: último valor contra los umbrales oficiales de la CHJ. */
  private async flowComponents(
    ...
    const rows = await this.db.execute<ObsRow>(sql`
   ...
   */
  private async rainObservedComponents(
    points: WatchPointSpec[],
    ...
    const rows = await this.db.execute<RainRow>(sql`
   ...
   */
  private async rainForecastComponents(
    station: VirtualStation,
    thresholds: Map<string, ThresholdSpec>,
    now: Date,
  ): Promise<RiskComponent[]> {
    const rows = await this.db.execute<ForecastRow>(sql`
      with latest as (
  ...
  private async alertsFor(station: VirtualStation, now: Date): Promise<StationRisk["alerts"]> {
    if (!station.aemetZone) return [];
    const rows = await this.db.execute<AlertRow>(sql`
