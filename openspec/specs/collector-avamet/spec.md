...
Las estaciones DEBEN registrarse con `kind='rain_gauge'`, fuente `avamet` y una nota en `meta` que indique que son amateur y bajo qué licencia se publican. El frontend DEBE mostrar la atribución AVAMET (CC BY-NC-ND 4.0) visible en pie global.

#### Scenario: Red amateur marcada
- **Dado** un dato de AVAMET
- **Entonces** su `meta` indica `amateur` y el pie de la web enlaza a `https://www.avamet.org` y a `https://creativecommons.org/licenses/by-nc-nd/4.0/deed.es`.
